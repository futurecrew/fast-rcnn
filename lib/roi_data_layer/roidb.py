...
def prepare_one_roidb_frcnn(roidb, proposal_file, num_classes):
    ...
    if candidiate_db == None and proposal_file != None:
        try:
            ...
            for i in xrange(10):
                try:
                    print 'trying %s' % proposal_file + str(i)
                    candidiate_db = leveldb.LevelDB(proposal_file + str(i))
                    break
                except:
                    print 'cannot use %s' % proposal_file + str(i)
...
def prepare_roidb_one_process(cpu_id, roidb, image_index, 
                              image_path_at,
                              start_index,
                              model_to_use,
                              proposal_file,
                              num_classes,
                              queue_child_to_parent):
    if model_to_use == 'rpn':
        bbox_class_counts = 0
    ...
    print 'len(image_index) : %s' % len(image_index)

    for i in xrange(len(image_index)):
        image_path = image_path_at(start_index + i)
        ...
        if i % 100 == 0:
            print 'processing image[%s] %s/%s' % (cpu_id, i, len(image_index))

    print 'processed image[%s] %s' % (cpu_id, (i+1))
    
    queue_data = {}
    queue_data['cpu_id'] = cpu_id
    queue_data['roidb'] = roidb
    queue_data['bbox_class_counts'] = bbox_class_counts
    queue_data['bbox_sums'] = bbox_sums
    queue_data['bbox_squared_sums'] = bbox_squared_sums
...
def prepare_roidb(imdb, model_to_use, proposal_file):
    ...
    if model_to_use == 'rpn':
        total_bbox_class_counts = 0
        total_bbox_sums = np.zeros((1, 4))
        total_bbox_squared_sums = np.zeros((1, 4))
    elif model_to_use == 'frcnn':
        total_bbox_class_counts = np.zeros((num_classes, 1)) + cfg.EPS
        total_bbox_sums = np.zeros((num_classes, 4))
        total_bbox_squared_sums = np.zeros((num_classes, 4))
        #MULTI_CPU_NO = 1
    ...
    total_data_no = len(imdb.image_index)
    ...
    for i in range(MULTI_CPU_NO):
        ...
        p = Process(target=prepare_roidb_one_process, args=(i, 
                                                            roidb[start_idx : end_idx], 
                                                            imdb.image_index[start_idx : end_idx],
                                                            imdb.image_path_at,
                                                            start_idx,
                                                            model_to_use, 
                                                            proposal_file,
    ...
    while True:
        ...
        print 'get finished_child %s' % queue_data['cpu_id']
        if finished_child_no == MULTI_CPU_NO:
            break
        else:
            continue
    ...
    for i in range(len(process_list)):
        for ret_value in ret_list:
            if ret_value['cpu_id'] == i:
                roidb[start_idx_list[i] : end_idx_list[i]] = ret_value['roidb']
                total_bbox_class_counts += ret_value['bbox_class_counts']
                total_bbox_sums += ret_value['bbox_sums']
                total_bbox_squared_sums += ret_value['bbox_squared_sums']
                break
    
    imdb.bbox_means = total_bbox_sums / total_bbox_class_counts
    imdb.bbox_stds = np.sqrt(total_bbox_squared_sums / total_bbox_class_counts - imdb.bbox_means ** 2)
